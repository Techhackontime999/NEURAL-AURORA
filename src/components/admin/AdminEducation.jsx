...
export default function AdminEducation() {
  ...
  async function handleDelete(id) {
    if (confirm('Delete?')) { await deleteEducation(id); load() }
  }
          ...
          <div key={item.id} className="rounded-lg border border-white/5 bg-white/[0.02] p-4">
            {editingId === item.id ? (
              ...
              <div className="flex items-start justify-between">
                <div>
                  <h3 className="text-sm font-medium text-white">{item.degree}</h3>
                  <p className="text-xs text-neural-400">{item.school} — {item.year}</p>
                </div>
